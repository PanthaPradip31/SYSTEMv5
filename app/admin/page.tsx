...
export default function AdminDashboard() {
  ...
  const knockedPlayers = teams.reduce((sum, team) => sum + (team.players || []).filter((p) => !p.isSub && p.status === "knocked").length, 0)
  ...
  const triggerSampleMVP = () => {
    ...
    toast.success("Broadcast cue: Triggered MVP cinematic stinger!")
  }
  ...
  const triggerSampleWWCD = () => {
    ...
    toast.success("Broadcast cue: Triggered Chicken Dinner celebration overlay!")
  }
  ...
  const triggerSampleEpicKill = () => {
    ...
    toast.success("Broadcast cue: Injected AWM elimination banner feed!")
  }

  const handleResetAliveCounts = () => {
    teams.forEach(t => {
      const updatedPlayers = t.players.map(p => ({ ...p, status: "alive" as const }))
      updateTeam(t.id, { players: updatedPlayers })
    })
    getRealtimeSync()?.broadcast("FORCE_REFRESH", {})
    toast.success("Lobby status: Reset all player counts to alive successfully!")
  }
  ...
  return (
    <div className="p-6 space-y-6 max-w-[1600px] mx-auto relative min-h-screen">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-white/5 pb-5 stagger-1">
        ...
        {role === "caster-locked" && (
          <div className="flex items-center gap-2 px-4 py-2.5 bg-zinc-950 border border-white/5 rounded-xl self-start md:self-auto shadow-2xl">
            <EyeIcon className="w-4 h-4 text-gold" />
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 stagger-1">
        ...
        <Card className="bg-zinc-950/40 backdrop-blur-md border border-white/5 shadow-xl hover:border-gold/20 hover:shadow-[0_0_20px_rgba(218,165,32,0.06)] transition-all duration-300">
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            <CardTitle className="text-[10px] uppercase font-bold tracking-widest text-muted-foreground">Deployed Teams</CardTitle>
        ...
        <Card className="bg-zinc-950/40 backdrop-blur-md border border-white/5 shadow-xl hover:border-emerald-500/20 hover:shadow-[0_0_20px_rgba(16,185,129,0.06)] transition-all duration-300">
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            <CardTitle className="text-[10px] uppercase font-bold tracking-widest text-muted-foreground">Combatants Alive</CardTitle>
        ...
        <Card className="bg-zinc-950/40 backdrop-blur-md border border-white/5 shadow-xl hover:border-amber-500/20 hover:shadow-[0_0_20px_rgba(245,158,11,0.06)] transition-all duration-300">
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            <CardTitle className="text-[10px] uppercase font-bold tracking-widest text-muted-foreground">Knocked Down</CardTitle>
        ...
        <Card className="bg-zinc-950/40 backdrop-blur-md border border-white/5 shadow-xl hover:border-red-500/20 hover:shadow-[0_0_20px_rgba(239,68,68,0.06)] transition-all duration-300">
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            <CardTitle className="text-[10px] uppercase font-bold tracking-widest text-muted-foreground">Total Match Kills</CardTitle>
      ...
      {role === "caster-locked" && (
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-6 stagger-2">
          ...
          <div className="xl:col-span-2 space-y-6">
            <Card className="bg-zinc-950/45 backdrop-blur-md border border-white/5 shadow-2xl rounded-2xl overflow-hidden relative transition-all duration-300 hover:border-white/10">
              <div className="h-[2px] bg-gradient-to-r from-transparent via-gold to-transparent opacity-40" />
              <CardHeader className="border-b border-white/5 bg-black/30">
                <CardTitle className="text-xs uppercase tracking-widest text-neutral-200 flex items-center gap-2">
                  ...
                  <tbody>
                    {teams.map((team) => (
                      team.players.map((p, idx) => (
                        ...
                        >
                          {idx === 0 ? (
                            <td className="py-3 px-5 font-bold text-neutral-200 align-top" rowSpan={team.players.length}>
                              <div className="flex items-center gap-2 mt-0.5">
                                <span className="w-1.5 h-6 rounded" style={{ backgroundColor: team.color, boxShadow: `0 0 8px ${team.color}` }} />
                                <span className="truncate max-w-[120px] tracking-wide font-black uppercase text-xs">{team.name}</span>
                              </div>
                            </td>
                          ) : null}
                          {idx === 0 ? (
                            <td className="py-3 px-5 font-mono font-black align-top text-xs mt-0.5" style={{ color: team.color }} rowSpan={team.players.length}>
                              {team.shortName}
                            </td>
                          ) : null}
                          ...
                          <td className="py-3.5 px-5">
                            <span className={cn(
                              ...
                              <span className={cn(
                                ...
                                p.status === "alive" ? "bg-emerald-400" : p.status === "knocked" ? "bg-amber-400 animate-ping" : "bg-red-400"
                              )} />
                              {p.status}
                            </span>
                          </td>
          ...
          <div className="space-y-6">
            ...
            <Card className="bg-zinc-950/45 backdrop-blur-md border border-white/5 shadow-xl overflow-hidden hover:border-white/10 transition-all duration-300">
              <div className="h-[2px] bg-gradient-to-r from-transparent via-red-500/30 to-transparent" />
              <CardHeader className="pb-3 border-b border-white/5 bg-black/30">
                <CardTitle className="text-xs uppercase font-bold tracking-wider text-glow flex items-center justify-between">
              ...
              <CardContent className="pt-4">
                <div 
                  ref={monitorRef}
                  className="w-full aspect-video bg-neutral-950 rounded-xl overflow-hidden border border-white/10 relative shadow-2xl"
                >
            ...
            <Card className="bg-zinc-950/45 backdrop-blur-md border border-white/5 shadow-xl hover:border-white/10 transition-all duration-300">
              <CardHeader className="border-b border-white/5 bg-black/30 pb-3">
              ...
              <CardContent className="pt-4 space-y-2.5 max-h-[300px] overflow-y-auto pr-2">
                {killFeed.map((k) => (
                  <div key={k.id} className="flex items-center justify-between p-3 bg-black/40 border border-white/5 rounded-xl text-xs hover:border-white/10 transition-all shadow-inner">
                    ...
                    <span className={cn(
                      "text-[9px] uppercase font-black font-mono tracking-widest pl-2 shrink-0 px-2 py-0.5 rounded-full border",
                      k.isKnock 
                ...
                {killFeed.length === 0 && (
                  <div className="text-center py-8 text-[9px] text-muted-foreground uppercase font-black tracking-widest p-4 rounded-xl border border-white/5 bg-black/20">
      ...
      {role === "director" && (
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-6 stagger-2">
          ...
          <div className="xl:col-span-2 space-y-6">
            ...
            <Card className="bg-zinc-950/45 backdrop-blur-md border border-white/5 shadow-xl overflow-hidden hover:border-white/10 transition-all duration-300">
              <div className="h-[2px] bg-gradient-to-r from-transparent via-gold to-transparent opacity-40" />
              <CardHeader className="bg-black/30 border-b border-white/5">
                <CardTitle className="text-xs uppercase tracking-widest text-white font-black">Director Control Center Channels</CardTitle>
                <CardDescription className="text-[10px] uppercase tracking-wider text-white/80 font-bold mt-0.5">Tactical setup panels and operational dashboard routing</CardDescription>
              </CardHeader>
              <CardContent className="grid sm:grid-cols-2 md:grid-cols-3 gap-3 pt-4">
                <Link href="/admin/match" className="block">
                  <Button className="w-full h-14 justify-start gap-3 bg-zinc-900 border border-white/5 hover:border-gold/30 hover:bg-gold/5 text-white hover:text-white text-[10px] font-black uppercase tracking-widest transition-all duration-300 rounded-xl relative pl-4 hover:shadow-[0_0_12px_rgba(218,165,32,0.06)] active:scale-[0.98]">
            ...
            <Card className="bg-zinc-950/45 backdrop-blur-md border border-white/5 shadow-xl overflow-hidden hover:border-white/10 transition-all duration-300">
              <div className="h-[2px] bg-gradient-to-r from-transparent via-gold to-transparent opacity-40" />
              <CardHeader className="bg-black/30 border-b border-white/5">
                <CardTitle className="text-xs uppercase tracking-widest text-neutral-200">Esports Stinger FX Trigger Deck</CardTitle>
                <CardDescription className="text-[10px] uppercase tracking-wider text-muted-foreground mt-0.5">Fire animated stinger widgets instantly to your OBS browser canvases</CardDescription>
              </CardHeader>
              <CardContent className="grid grid-cols-3 gap-4 pt-4">
                <Button 
                  onClick={triggerSampleMVP}
                  className="gradient-gold text-primary-foreground font-black text-[10px] tracking-widest uppercase h-12 hover:brightness-110 hover:shadow-[0_0_15px_rgba(218,165,32,0.35)] transition-all duration-300 hover:scale-[1.02] active:scale-[0.98] rounded-xl"
                >
                  🏆 Trigger MVP stinger
                </Button>
                <Button 
                  onClick={triggerSampleWWCD}
                  className="bg-[#d97706] hover:bg-[#b45309] text-white font-black text-[10px] tracking-widest uppercase h-12 hover:shadow-[0_0_15px_rgba(217,119,6,0.35)] transition-all duration-300 hover:scale-[1.02] active:scale-[0.98] rounded-xl border border-amber-500/30"
                >
                  🍗 Trigger WWCD stinger
                </Button>
                <Button 
                  onClick={triggerSampleEpicKill}
                  className="bg-[#991b1b] hover:bg-[#7f1d1d] text-white font-black text-[10px] tracking-widest uppercase h-12 hover:shadow-[0_0_15px_rgba(153,27,27,0.35)] transition-all duration-300 hover:scale-[1.02] active:scale-[0.98] rounded-xl border border-red-500/30"
                >
                  💀 Trigger Epic Wipe
                </Button>
              </CardContent>
            ...
            <Card className="bg-zinc-950/45 backdrop-blur-md border border-white/5 shadow-xl overflow-hidden hover:border-white/10 transition-all duration-300">
              <div className="h-[2px] bg-gradient-to-r from-transparent via-red-500/30 to-transparent" />
              <CardHeader className="bg-black/30 border-b border-white/5">
                <CardTitle className="text-xs uppercase tracking-widest text-red-400 font-bold">Lobby Database Override Knobs</CardTitle>
                <CardDescription className="text-[10px] uppercase tracking-wider text-muted-foreground mt-0.5">Emergency database overrides to reset standings metrics and wipe channels</CardDescription>
              </CardHeader>
              <CardContent className="flex flex-wrap gap-3 pt-4">
                <Button 
                  variant="outline" 
                  onClick={handleResetAliveCounts}
                  className="border-red-950 text-red-400 hover:bg-red-500/10 text-[10px] font-black uppercase tracking-widest gap-2 h-11 px-5 rounded-xl hover:border-red-500 transition-all duration-300 active:scale-[0.98]"
                >
                ...
                <Button 
                  variant="outline" 
                  onClick={() => getRealtimeSync()?.broadcast("FORCE_REFRESH", {})}
                  className="border-white/10 hover:bg-white/5 text-[10px] font-black uppercase tracking-widest h-11 px-5 rounded-xl transition-all duration-300 active:scale-[0.98]"
                >
          ...
          <div className="space-y-6">
            <Card className="bg-zinc-950/45 backdrop-blur-md border border-white/5 shadow-xl overflow-hidden hover:border-white/10 transition-all duration-300">
              ...
              <CardHeader className="bg-black/30 border-b border-white/5">
                <CardTitle className="text-xs uppercase tracking-widest text-neutral-200 font-bold flex items-center gap-2">
                ...
                <CardDescription className="text-[10px] uppercase tracking-wider text-muted-foreground mt-0.5">Copy ultra-low latency graphic widgets into your OBS canvases</CardDescription>
              </CardHeader>
              <CardContent className="space-y-4 pt-4">
                {obsSources.map((source) => (
                  <div key={source.url} className="space-y-2 p-3 bg-zinc-950 border border-white/5 rounded-xl hover:border-white/10 transition-all shadow-inner">
                    ...
                    <div className="flex items-center gap-2 pt-0.5">
                      <input
                        value={`${typeof window !== "undefined" ? window.location.origin : ""}${source.url}`}
                        readOnly
                        className="text-[9px] h-8 bg-black/60 border border-white/5 px-2.5 rounded-lg flex-1 font-mono select-all text-neutral-400 focus:outline-none"
                      />
                      <Button
                        variant="secondary"
                        size="sm"
                        onClick={() => {
                          navigator.clipboard.writeText(`${window.location.origin}${source.url}`)
                          toast.success(`Copied URL for ${source.name}!`)
                        }}
